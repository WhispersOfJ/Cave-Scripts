# ============================================================================
# __helpers — internal helpers for the cave-* functions (zsh port)
# ============================================================================
# zsh translations of __arr_api, __arr_api_url, __arr_api_key,
# __stack_arr_app, __plex_api, __plex_butler, __seerr_api, __nzbdav_api,
# __stack_containers (same contracts as the bash port).
# Sourced by cave-scripts.zsh before the cave-*.zsh files.
# ============================================================================
...
__arr_api_key() {
    ...
    if [ -z "$key" ]; then
        echo "API key for $1 not set (expected ${(U)1}_API_KEY uppercase in environment)" >&2
        return 1
    fi
    echo "$key"
}
