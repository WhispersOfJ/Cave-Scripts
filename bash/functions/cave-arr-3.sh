# ============================================================================
# cave-arr-3.sh — arr commands (part 3) + summary commands
# ============================================================================
...
# complete: -y|--yes|--dry-run
    ...
    if [ "$stopped" = true ]; then
        ...
        docker compose -f "$repo/docker-compose.yml" start radarr >/dev/null 2>&1 \
            || fmt_warning "radarr did not restart cleanly — run cave-restart-all"
    fi
...
# complete: -y|--yes|--dry-run
    ...
    if [ "$stopped" = true ]; then
        ...
        docker compose -f "$repo/docker-compose.yml" start sonarr >/dev/null 2>&1 \
            || fmt_warning "sonarr did not restart cleanly — run cave-restart-all"
    fi
