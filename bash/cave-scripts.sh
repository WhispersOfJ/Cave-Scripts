#!/usr/bin/env bash
# ============================================================================
# cave-scripts.sh — Cave-Scripts bash loader (former bearcave-bash.sh)
# ============================================================================
# Sourced from ~/.bashrc (interactive shells only). Provides:
...
#   4. cave-* function library: sources every bash/functions/cave-*.sh
#      (renamed per D17; the stack-* alias layer ships in _aliases.sh, D21).
# ============================================================================

# ----------------------------------------------------------------------------
# 0. Locate Cave-Scripts (this file lives at <repo>/bash/) and the thebearcave
#    checkout that owns the stack .env. Cave-Scripts is path-independent of
#    thebearcave (§5.4): CAVE_SCRIPTS_DIR comes from this file's own location;
#    the .env resolves to thebearcave (BEARCAVE_REPO_DIR honored, default
#    $HOME/cave).
# ----------------------------------------------------------------------------
_cave_self="${BASH_SOURCE[0]}"
_cave_dir="$(cd "$(dirname "$_cave_self")" && pwd)"
...
# ----------------------------------------------------------------------------
# 5. cave-* function library (bash translations of the fish functions)
#    Helpers first (__*.sh), then user commands (cave-*.sh), then the
#    permanent stack-* alias layer (_aliases.sh, D21), then generated
#    tab-completions.
# ----------------------------------------------------------------------------
_cave_fn_dir="$CAVE_SCRIPTS_DIR/functions"
if [ -d "$_cave_fn_dir" ]; then
    for _f in "$_cave_fn_dir"/__*.sh "$_cave_fn_dir"/cave-*.sh; do
        [ -f "$_f" ] && source "$_f"
    done
    unset _f
fi
...
_cave_aliases="$CAVE_SCRIPTS_DIR/_aliases.sh"
[ -f "$_cave_aliases" ] && source "$_cave_aliases"
unset _cave_aliases

_cave_comp="$CAVE_SCRIPTS_DIR/completions/cave-completions.sh"
[ -f "$_cave_comp" ] && source "$_cave_comp"
unset _cave_comp
